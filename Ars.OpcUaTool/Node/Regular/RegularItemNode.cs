...
namespace Ars.Common.OpcUaTool.Node.Regular
{
    ...
    public class RegularItemNode : NodeClass, IComparable<RegularItemNode>
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string GetFromDbIndex(string address)
        {
            if(int.TryParse(System.Text.RegularExpressions.Regex.Replace(address, @"[^0-9]+", ""), out int addr))
            {
                int newAddr = addr + Index / 2;

                address = address.Replace($"{addr}", $"{newAddr}");
            }
